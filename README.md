...
- Web Cloud 경로 시작점은 섹션명과 분리된 `내 드라이브`를 사용하고, 현재 폴더명 영역 안에서 경로를 함께 보여줘 상단 용어가 불필요하게 겹치지 않게 유지한다
- Web Cloud 로딩 신호는 상단 헤더 한 곳으로만 모으고, 폴더 전환 중에는 지연된 작은 indicator만 보여 일관된 전환감을 유지한다
- Web Cloud 검색창은 실제 검색이 가능한 `내 파일` 섹션에서만 노출하고, 최근/공유됨/중요/휴지통에는 표시하지 않는다
...
- Web Cloud 섹션 전환 시 이전 섹션 목록을 재사용하지 않고 즉시 새 섹션 기준으로 초기화해 `내 파일` 내용이 `휴지통`에 남아 보이지 않게 유지한다

...
- Web Cloud 폴더 화면은 `Cloud` 공통 제목 아래 현재 폴더명 영역에서 경로를 함께 표시하고, 마지막 폴더명에서 현재 폴더 이름 변경/중요 표시 같은 컨텍스트 액션을 바로 실행할 수 있다
...
- Web Cloud 섹션 전환 시 이전 섹션 목록을 재사용하지 않고 즉시 새 섹션 기준으로 초기화해 `내 드라이브` 내용이 `휴지통`에 남아 보이지 않게 유지한다
- Web Cloud 휴지통 선택 바는 선택 항목 `복원/삭제` 전용으로만 쓰고, 전체 `휴지통 비우기`는 비선택 상태의 전역 액션으로만 노출해 전체 삭제와 선택 삭제를 분리한다
...
- Web/Desktop Cloud 붙여넣기(복사/이동)는 실행 직후 5초 실행 취소를 제공하고, Undo는 서버 발급 단기 `undo_token` 과 현재 파일 상태 일치 검증으로만 허용한다
- Cloud 영구 삭제/붙여넣기 취소 시 원본 파일과 썸네일을 함께 정리하고, Gallery 썸네일 조회도 활성 파일 존재를 먼저 확인한다

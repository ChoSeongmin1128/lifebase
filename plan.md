...
## 9. Cloud 파일 드래그 이동 계획 (웹 1차)
1. 범위
`내 파일` 섹션에서 파일을 폴더로 드래그해 이동하는 기능을 1차 적용한다.

2. 구현 레이어
`CloudRepository`-`ManageCloudUseCase`-`useCloudActions`-`cloud/page.tsx` 순으로 `moveFile` 흐름을 연결한다.

3. 상호작용 규칙
파일만 드래그 가능하고 폴더만 드롭 타깃으로 허용한다.

4. 피드백 규칙
드롭 가능한 폴더에는 hover 강조를 적용하고 이동 중에는 중복 요청을 차단한다.

5. 예외 처리
같은 폴더로의 이동은 무시하고 실패 시 콘솔 오류와 사용자 메시지로 원인을 알린다.

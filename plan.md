...
- 폴더 내부 상단은 breadcrumb + 현재 폴더명 헤더를 사용하고, 로컬 경로 상태를 우선 적용해 탐색 중 경로 흔들림을 줄인다.
- 경로 시작점은 `내 파일` 같은 섹션명과 구분되는 `보관함`을 사용하고, breadcrumb 행에는 현재 폴더를 반복하지 않아 서비스명/섹션명/경로명이 같은 단어로 겹치지 않게 유지한다.
...
- Web Cloud 섹션 전환 시 이전 섹션 목록을 즉시 비워 `내 파일` 내용이 `휴지통`에 남아 보이는 상태를 방지한다.
...
4. Cloud
`내 파일`의 파일/폴더 삭제는 휴지통 이동 전에 같은 5초 Undo 토스트를 제공한다.

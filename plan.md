...
- 경로 시작점은 `내 파일` 같은 섹션명과 구분되는 `내 드라이브`를 사용하고, 현재 폴더명 영역 안에서 경로를 함께 보여줘 서비스명/섹션명/경로명이 같은 단어로 과하게 겹치지 않게 유지한다.
- Cloud 로딩 신호는 상단 헤더 한 곳으로만 두고, 짧은 이동에는 spinner를 지연 표시해 전환 감각을 일관되게 유지한다.
- Cloud 검색창은 실제 검색이 가능한 `내 파일`에서만 노출하고, 나머지 섹션에서는 숨겨 no-op UI를 제거한다.

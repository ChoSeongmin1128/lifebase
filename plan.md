...
- 사이드바 보조 내비는 Cloud/Calendar/Todo에만 적용하고, Home/Gallery/Settings는 직접 진입과 페이지 내부 상태 UI를 우선한다.
...
- 현재 폴더 삭제는 경로 헤더에서 바로 실행하되, 삭제 직후 상위 폴더에서 항목을 즉시 숨기고 같은 5초 Undo 토스트로 상위 폴더에 머문 채 항목 복구를 지원한다.
- 회귀 방지 규칙: 라우트 전환이 섞인 Undo는 컴포넌트 로컬 ref/state가 아니라 라우트 교체 뒤에도 유지되는 세션 상태로 관리한다.
- `휴지통 비우기`는 실행 전 확인 다이얼로그를 한 번 더 보여주고, 확정 후에도 같은 5초 Undo 토스트를 유지한다.
...
1. 공통 보조 내비
Web/Desktop 사이드바는 Cloud/Calendar/Todo만 서비스별 보조 내비를 같은 위치에 확장한다. Home/Gallery/Settings는 상위 항목 직접 진입만 제공한다.
...
3. 목적
보조 내비 또는 페이지 내부 탭 상태와 URL을 일치시켜 새로고침, 딥링크, Desktop 래핑 환경에서 같은 문맥을 복원한다.

4. Settings 구조
Web/Desktop Settings와 Mobile Settings는 모두 페이지 내부 탭을 사용하되 섹션 의미는 동일하게 유지한다.
...
- `?focus=summary|calendar|todo|files|storage`는 Home 직접 링크/새로고침 복원용으로 유지한다
- 빠른 액션 딥링크 연결

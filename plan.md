...
- Todo 목록 사이드바는 카드형 항목, 우측 진행 배지, 하단 출처/완료 보조 정보로 위계를 나누고 활성 강조는 면 중심으로 처리한다.
...
- Cloud `새 파일` 입력의 기본 확장자는 `txt`로 두고, 입력을 다시 열어도 같은 기본값으로 복원한다.
...
## 16. Google Calendar API 에러 매핑 표준화 계획
1. 범위
